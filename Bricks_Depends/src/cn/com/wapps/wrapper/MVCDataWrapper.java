...
import cn.com.utils.FormParameterSetter;
    ...
    private Object invoke(Class<?> clazz, String parentFieldName, Map<String, Object> parameters, int idx) throws Throwable {
        ...
        for (Field field : fields) {
            ...
            if (TypeInitialization.isCommonType(field.getType())) {
                TypeContentRefecter.setValue(value, field.getName(), new Class[]{field.getType()}, FormParameterSetter.getParam(parameterKey, field.getType(), parameters, idx, partten));
                continue;
            }

            // 处理数组类型
            if (field.getType().isArray()) {
                TypeContentRefecter.setValue(value, field.getName(), new Class[]{field.getType()}, FormParameterSetter.getParamArray(parameterKey, field.getType().getComponentType(), parameters, partten));
                continue;
            }
            ...
            if ((field.getType().isInterface() && field.getType().equals(List.class))) {
                ...
                for (Map.Entry<String, Object> entry : parameters.entrySet()) {
                    ...
                    if (entry.getKey().equals("selectInput")) {
                ...
                if (checkedBound) {
                    if (checkedObjects != null && contentObjects != null) {
                        ...
                        for (int idx1 = 0; idx1 < checkedObjects.length; idx1++) {
                            contents.add(invoke(contentClass, field.getName(), parameters, LocalConvert.getInstance().convertStringToInteger((String) checkedObjects[idx1])));
                        }
                    }
