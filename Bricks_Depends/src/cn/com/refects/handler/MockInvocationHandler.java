...
public class MockInvocationHandler implements InvocationHandler {
     ...
     * @throws Throwable
     */
    public Object invokeByFile(Method method) throws Throwable {

        // 排除void类
        if (method.getReturnType().equals(Void.class)) {
            return null;
        }
        String path = StringUtil.joinString(SpliterSymbol, method.getDeclaringClass().getSimpleName(), method.getName());
        List results = null;
        // 返回通用类初始化
        if (TypeInitialization.isCommonType(method.getReturnType())) {
            results = DataProviderUtil.INSTACE.getData(path, method.getReturnType());
        ...
        if ((method.getReturnType().isInterface() && method.getReturnType().equals(List.class))) {
            ...
            DataProviderUtil.INSTACE.setParentPath(contentPath);
